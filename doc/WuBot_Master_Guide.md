...
**转化后的 CSV 长什么样？**：
这 36 个浮点数（3位移 + 4朝向 + 29关节角）被压平到一行内，表示极短一瞬间（比如 0.02s，50Hz）下机器狗应当展现的完整姿势。
```csv
# Base_X, Base_Y, Base_Z, Quat_X, Quat_Y, Quat_Z, Quat_W, left_hip_pitch, left_hip_roll, ..., right_elbow_pitch
0.015, -0.002, 0.785, 0.0, 0.0, 0.0, 1.0, -0.35, 0.12, ..., 0.0
0.017, -0.001, 0.784, 0.01, -0.01, 0.003, 0.99, -0.34, 0.13, ..., 0.0
...每一行就是机器人在模仿学习时，下一帧必须追赶的标准靶目标(Target Pose)...
```
...
3. **格式合规化**：输出 Numpy 多维张量数组（含 `root_pos`, `root_quat_wxyz`, `joint_pos`, `joint_vel`），供 Isaac Lab 环境（ManagerBasedRLEnv）在 GPU 内显存上零拷贝高速采样！

**重放核心逻辑**（在 Isaac Lab 中运行）：
```python
# 核心逻辑（简化版）
