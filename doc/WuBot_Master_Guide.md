...
- [武术机器人（WuBot）完全精通指南](#武术机器人wubot完全精通指南)
  - [目录](#目录)
- [第一部分：项目总览](#第一部分项目总览)
  - [1.1 项目目标](#11-项目目标)
...
- [第二部分：数据流水线](#第二部分数据流水线)
  - [2.1 数据来源：CMU Motion Capture](#21-数据来源cmu-motion-capture)
    - [ASF 骨骼树的真实人体解剖学对应](#asf-骨骼树的真实人体解剖学对应)
    - [🌟 核心原理解析：CMU 数据如何与算法相匹配？](#-核心原理解析cmu-数据如何与算法相匹配)
  - [2.2 阶段一：CMU ASF+AMC → CSV](#22-阶段一cmu-asfamc--csv)
    - [CMU → G1 关节映射的具体机制](#cmu--g1-关节映射的具体机制)
    - [符号（sign）为什么有 +1 和 -1？](#符号sign为什么有-1-和--1)
    - [坐标系转换（仅基座）](#坐标系转换仅基座)
    - [尺度缩放（仅基座位移）](#尺度缩放仅基座位移)
    - [CSV 每行 36 列的完整含义](#csv-每行-36-列的完整含义)
  - [2.3 阶段二：CSV → NPZ (生成算法直接可用的示教数据)](#23-阶段二csv--npz-生成算法直接可用的示教数据)
    - [CSV 有什么、缺什么？](#csv-有什么缺什么)
    - [缺失数据如何生成？——Isaac Sim FK + 有限差分](#缺失数据如何生成isaac-sim-fk--有限差分)
    - [帧率转换：CMU 120Hz → NPZ 50Hz](#帧率转换cmu-120hz--npz-50hz)
  - [2.4 数据质量关键：关节限位钳位](#24-数据质量关键关节限位钳位)
  - [2.5 NPZ 文件内容详解](#25-npz-文件内容详解)
- [第三部分：机器人物理模型](#第三部分机器人物理模型)
  - [3.1 G1 29DOF 关节定义](#31-g1-29dof-关节定义)
    - [29DOF 的含义与分组](#29dof-的含义与分组)
    - [SIM 关节顺序](#sim-关节顺序)
...
- [第四部分：训练环境架构](#第四部分训练环境架构)
  ...
  - [4.4 观测空间设计](#44-观测空间设计)
    - [Policy 观测（Actor 网络输入）](#policy-观测actor-网络输入)
    - [Critic 观测（Critic 网络输入，含特权信息）](#critic-观测critic-网络输入含特权信息)
  - [4.5 动作空间设计](#45-动作空间设计)
  - [4.6 Domain Randomization（域随机化）](#46-domain-randomization域随机化)
    - [Startup 随机化（初始化时一次性设置）](#startup-随机化初始化时一次性设置)
    - [Interval 随机化（训练中周期性触发）](#interval-随机化训练中周期性触发)
    - [Resample 随机化（episode 开始时）](#resample-随机化episode-开始时)
...
- [第六部分：奖励函数设计（核心）](#第六部分奖励函数设计核心)
  ...
  - [6.5 关节位置跟踪奖励（关键！）](#65-关节位置跟踪奖励关键)
    - [为什么 body\_pos/ori 不够？为什么还需要 joint\_pos？](#为什么-body_posori-不够为什么还需要-joint_pos)
  ...
  - [6.10 奖励权重总表（v9）](#610-奖励权重总表v9)
    - [正奖励（跟踪目标，总和 = 10.0）](#正奖励跟踪目标总和--100)
    - [负奖励（正则化 + 安全）](#负奖励正则化--安全)
- [第七部分：终止条件设计](#第七部分终止条件设计)
  - [7.1 终止条件的作用](#71-终止条件的作用)
  - [7.2 锚点位置/朝向终止](#72-锚点位置朝向终止)
    - [anchor\_pos\_z\_only](#anchor_pos_z_only)
    - [anchor\_ori](#anchor_ori)
  - [7.3 末端执行器位置终止](#73-末端执行器位置终止)
  - [7.4 非法接触终止（v9 新增）](#74-非法接触终止v9-新增)
    - [26 个非法接触身体](#26-个非法接触身体)
    - [threshold = 100N 的选择](#threshold--100n-的选择)
  - [7.5 终止条件总表（v9）](#75-终止条件总表v9)
- [第八部分：自适应采样机制](#第八部分自适应采样机制)
  - [8.1 为什么需要自适应采样](#81-为什么需要自适应采样)
  - [8.2 算法原理](#82-算法原理)
    - [时间分箱（Binning）](#时间分箱binning)
    - [失败计数与 EMA 更新](#失败计数与-ema-更新)
    - [核平滑](#核平滑)
    - [采样概率](#采样概率)
    - [采样流程](#采样流程)
...
- [第十一部分：参数调优方法论](#第十一部分参数调优方法论)
  - [11.1 奖励权重调优原则](#111-奖励权重调优原则)
    - [原则 1：正奖励总和要远大于负奖励量级](#原则-1正奖励总和要远大于负奖励量级)
    - [原则 2：σ 参数反映物理量级](#原则-2σ-参数反映物理量级)
    - [原则 3：权重代表优先级](#原则-3权重代表优先级)
    - [原则 4：先粗后细](#原则-4先粗后细)
  - [11.2 终止阈值调优原则](#112-终止阈值调优原则)
    - [原则 1：终止阈值 = "不可恢复" 的边界](#原则-1终止阈值--不可恢复-的边界)
    - [原则 2：先松后紧](#原则-2先松后紧)
    - [原则 3：观察 episode 长度](#原则-3观察-episode-长度)
  - [11.3 常见问题诊断与解决](#113-常见问题诊断与解决)
    - [问题 1：膝盖着地 / 身体接触地面](#问题-1膝盖着地--身体接触地面)
    - [问题 2：胳膊拧弯 / 关节配置错误](#问题-2胳膊拧弯--关节配置错误)
    - [问题 3：训练崩溃 / episode 很短](#问题-3训练崩溃--episode-很短)
    - [问题 4：动作抖动 / 不平滑](#问题-4动作抖动--不平滑)
    - [问题 5：策略忽略某些身体部位](#问题-5策略忽略某些身体部位)
...
- [附录](#附录)
  ...
  - [B. 碰撞体列表](#b-碰撞体列表)
    - [30 个 Articulation Body（PhysX 暴露的刚体）](#30-个-articulation-bodyphysx-暴露的刚体)
    - [被 PhysX 合并的固定关节链接（不在 articulation body 列表中！）](#被-physx-合并的固定关节链接不在-articulation-body-列表中)
  - [C. 关键公式汇总](#c-关键公式汇总)
    - [指数核奖励](#指数核奖励)
    ...
    - [自适应采样 EMA](#自适应采样-ema)
    - [角速度（SO3 导数）](#角速度so3-导数)
    - [yaw-only 坐标变换](#yaw-only-坐标变换)
...
- **`.asf`** (Acclaim Skeleton File)：骨骼定义文件
  ...
  - **长什么样（文本结构）**：描述人体各部件的几何拓扑和转动极限。可以将其理解为**骨骼系统的一种早期“Schema 定义”或类似 YAML/JSON 格式的数据模型描述文件**，它定义了一套树状的人体骨骼装配规则。
    ```text
    begin
       id 1
       name lfemur       # 例如：左大腿
       ...
       length 17.5       # 骨骼长度
       axis 0 0 20 XYZ
       dof rx ry rz      # 支持旋转的自由度 (Roll, Pitch, Yaw)
       limits -160.0 20.0 -70.0 70.0 -90.0 90.0
    end
    ```

- **`.amc` 与 `.asf` 的直接关系（分离式数据结构）**：
  它们共同组成了一套完整的动画。`.asf`（骨骼定义档）相当于类（Class）或模型表定义；而 `.amc`（逐帧动作档）则相当于使用该类的实例数据（Instance Values）。`.amc` 描述的是：**在 `.asf` 中定义的那些原本静态的骨架，在真实运动过程中，随着每一帧时间推移而发生的绝对空间位移（Root）与关节旋转角度的数据轨迹。** 
  > 💡 这里的关键原理是：在整个骨架树中，只有**基座（Root/Pelvis）才有资格记录绝对方位（XYZ 平移 + 欧拉角朝向）**。因为其他的骨头都是直接或间接地像锁链一样连接在这个基座之上的，所以它们**只能记录相对上一级骨头的转动（即关节旋转角度）**。只有将 `.asf` 里固定的骨骼长度配置，加上 `.amc` 里这层层累加传递下来的各个关节旋转角度，通过计算机图形学所谓的正向运动学（FK）运算，最终才能推算出每一个非根节点（例如手尖或脚跟）在真实世界三维空间中所处的绝对位姿。这也是下一步为什么必须要写代码算平移的原因。

  > 📘 **科普拓展：正向运动学（FK）与反向运动学（IK）**
  > - **正向运动学 (Forward Kinematics, FK)**：推导逻辑是**由“角度”推算“位置”**。已知所有串联关节当前的旋转角度和骨头长度，正向一层层累加，算出肢体末端（如手掌/脚尖）在三维空间中的绝对坐标。这就是我们在推导 `.amc` 数据时做的事——比如你命令自己的肩膀转30度，手肘弯45度，大脑能够正向算出你的手挥到了哪儿。
  > - **反向运动学 (Inverse Kinematics, IK)**：推导逻辑是**由“位置”反求“角度”**。已知手掌/脚尖想要到达的目标空间位置（XYZ），反过来求解各个关节应该怎么转动、转多少度才能恰好摸到那个点。比如你命令机器人“伸手去拿桌上的水杯”。IK 在数学求解上通常非常复杂，因为往往有几十种甚至无数种不同的扭曲姿势（或者无解）都能碰到同一个坐标点。此时往往需要优化算法介入。

  `.amc` 文件中引用的名字（如 `lfemur`），都是在 `.asf` 根据层次结构事先声明过的。这种分离让同一个 `.asf` 骨架档案，可以完美地应用在无限种 `.amc` 动画数据中复用。

- **`.amc`** (Acclaim Motion Capture)：逐帧运动数据
  - 每帧每个关节的旋转角度
  - **长什么样（文本结构）**：只记录动作本身，以帧（Frame）为单位分隔：
    ```text
    1                    # 第 1 帧
    root 0.0 12.5 0.0 0.0 0.0 0.0    # 骨盆(基座)的XYZ位移与欧拉角旋转
    lfemur 1.2 -5.5 10.3             # 左大腿在这瞬间的各个旋转自由度数值
    ...
    2                    # 第 2 帧
    root 0.05 12.4 0.12 0.1 0.05 0.0
    lfemur 1.4 -5.4 10.5
    ...
    ```
...
ASF 骨骼定义**来源于真实人体骨骼**，骨骼名称直接使用医学/解剖学术语（femur=股骨、tibia=胫骨、humerus=肱骨、radius=桡骨等）。但相比真实人体有大幅简化：
...
```
                    ...
                    lhipjoint       rhipjoint
                       |               |
                    lfemur           rfemur          ← 大腿骨 (3DOF: rx,ry,rz)
                       |               |
                    ltibia           rtibia          ← 小腿/胫骨 (1DOF: rx)
                       |               |
                     lfoot            rfoot          ← 脚 (2DOF: rx,rz)
                       |               |
                    ltoes             rtoes           ← 脚趾
                    
                            root
                             |
                         lowerback                   ← 腰椎 (3DOF: rx,ry,rz)
                             |
                         upperback                   ← 胸椎
                             |
                          thorax                     ← 胸腔
                        /    |    \
                 lclavicle   |   rclavicle           ← 锁骨
                    |    lowerneck    |
                 lhumerus    |    rhumerus           ← 上臂/肱骨 (3DOF: rh,ry,rz)
                    |    upperneck    |
                 lradius     |    rradius             ← 前臂/桡骨 (1DOF: rx)
                    |      head       |
                 lwrist             rwrist            ← 手腕 (1DOF: ry)
                    |                 |
                 lhand              rhand             ← 手 (2DOF: rx,rz)
                   / \               / \
             lfingers lthumbs  rfingers rthumbs      ← 手指/拇指（G1 无对应）
```
...
| 真实人体 | ASF 模型 | 简化方式 |
|---------|---------|---------|
| 脊柱 33 节椎骨 | 3 根骨骼 (lowerback, upperback, thorax) | 大幅简化 |
| 颈椎 7 节 | 2 根 (lowerneck, upperneck) | 简化 |
| 每只手 27 块骨 | 3 根 (hand, fingers, thumb) | 极度简化 |
| 每只脚 26 块骨 | 2 根 (foot, toes) | 极度简化 |
| 髋关节（球窝关节） | femur 3DOF (rx,ry,rz) | ✅ 准确 |
| 膝关节（主要屈伸） | tibia 1DOF (rx) | ✅ 准确 |
| 肩关节（球窝关节） | humerus 3DOF (rh,ry,rz) | ✅ 准确 |
| 肘关节（主要屈伸） | radius 1DOF (rx) | ✅ 准确 |

> 💡 每个受试者的 ASF 不同——骨骼长度和方向是从真人身上测量的，反映了**该受试者（Subject #135）**的实际身体比例。这也是为什么 `csv_to_npz.py` 需要 `calibrate_scale()` 从骨骼腿长自动校准缩放比例。

### 🌟 核心原理解析：CMU 数据如何与算法相匹配？
要让原始的火柴人动捕数据，变成强化学习（PPO）算法能直接利用的**参照轨迹（Reference Trajectories，类似模仿学习的示范数据）**，中间必须经过“空间对齐”、“拓扑映射”和“物理可行性验证”三个核心步骤。这也正是接下来介绍的阶段一和阶段二的主线逻辑：

1. **坐标系对齐 (Coordinate Alignment)**：
   CMU 动捕的坐标系是典型的计算机图形学标准：**Y轴向上 (Y-up)**、Z轴向后、X轴向右。
   强化学习训练环境（Isaac Lab）是物理学标准：**Z轴向上 (Z-up)**、X轴向前、Y轴向左。
   因此，算法处理的第一步是对基座（Root/Base）的空间位移和四元数极其严格的旋转变换。

2. **拓扑与动力学映射 (Topology & Kinematics Retargeting)**：
   CMU 受试者有 30 根骨骼（包含锁骨、脚尖等机器人没有的关节），而 G1 机器人有 29 个驱动关节（且自由度受到机械限制）。
   我们需要编写**运动学映射逻辑**：例如将 CMU 的 `lfemur` (左大腿) 旋转角度通过公式，映射分解到 G1 机器人的 `left_hip_pitch` 和 `left_hip_roll` 这两台电机的目标角度上。

3. **速度推导与平滑 (Velocity Derivation & Smoothing)**：
   仅仅算出了各关节点的位置坐标是不够的！**RL 算法（如 PPO 的奖励函数）不仅需要机器人的姿势（位置），还需要考察动力学状态（基座线速度/角速度、各个关节的角速度）**。几何映射出来的数据必须经过平滑、并在时间轴上进行求导运算得出速度数据，最后打包成标准数组供算法随机采样。
...
ASF 骨骼树 + AMC 逐帧关节角
        │
        ▼
  关节映射：CMU bone 多轴旋转 → G1 单轴关节角
  (手动定义的 CMU_TO_G1_MAPPING 映射表)
        │
        ▼
  坐标系转换（仅基座）：CMU(Y-up) → Isaac(Z-up)
        │
        ▼
  尺度缩放（仅基座位移）：真人体型 → G1 机器人尺寸
        │
        ▼
  关节限位裁剪：clamp 到 URDF 物理极限
        │
        ▼
  CSV 输出 (每行 36 列 = 1帧)
```
...
映射的核心是一张**手动定义的 29 条映射表** `CMU_TO_G1_MAPPING`，每条格式为：

```python
(CMU骨骼名, 欧拉角轴索引, 符号, G1关节索引)
# 例如：
("lfemur",  0, -1.0, 0),   # lfemur 的 X轴(0)旋转 × (-1) → G1 joint[0] = left_hip_pitch
```

**映射原理**：
- CMU 的每根骨骼可以有 1~3 个旋转分量（欧拉角 rx, ry, rz）
- G1 的每个关节只有 **1 个自由度**（单轴旋转）
- 映射 = 从 CMU 骨骼的欧拉角中，**逐轴提取角度 → 乘以符号修正 → 转为弧度 → 赋给 G1 对应关节**

> 一根 CMU 骨骼的 3 个旋转分量可以映射到 **多个** G1 关节。例如 `lfemur` 的 rx/ry/rz 分别映射到 hip_pitch/hip_yaw/hip_roll 三个独立电机。

**具体映射举例**（以 `lfemur` 为例）：

```
AMC 数据:  lfemur  -15.3°(rx)  2.1°(ry)  8.7°(rz)

提取与转换:
  rx(-15.3°) × sign(-1) → +15.3° → 0.267 rad → G1 joint[0] = left_hip_pitch
  rz( 8.7°)  × sign(-1) → -8.7°  → -0.152 rad → G1 joint[1] = left_hip_roll
  ry( 2.1°)  × sign(+1) → +2.1°  → 0.037 rad → G1 joint[2] = left_hip_yaw
```

**完整映射表**：

| CMU 骨骼 | 部位 | 轴 | 符号 | → G1 关节 |
|---------|------|-----|------|----------|
| `lfemur` | 左大腿 | X,Z,Y | -1,-1,+1 | → hip_pitch, hip_roll, hip_yaw |
| `ltibia` | 左小腿 | X | +1 | → knee |
| `lfoot` | 左脚 | X,Z | +1,+1 | → ankle_pitch, ankle_roll |
| `rfemur` | 右大腿 | X,Z,Y | -1,-1,-1 | → hip_pitch, hip_roll, hip_yaw |
| `rtibia` | 右小腿 | X | +1 | → knee |
| `rfoot` | 右脚 | X,Z | +1,-1 | → ankle_pitch, ankle_roll |
| `lowerback` | 腰椎 | Y,Z,X | +1,+1,-1 | → waist_yaw, waist_roll, waist_pitch |
| `lhumerus` | 左上臂 | X,Z,Y | -1,+1,+1 | → shoulder_pitch, roll, yaw |
| `lradius` | 左前臂 | X | +1 | → elbow |
| `lwrist` | 左手腕 | Y | +1 | → wrist_roll |
| `lhand` | 左手 | X,Z | -1,+1 | → wrist_pitch, wrist_yaw |
| `rhumerus` | 右上臂 | X,Z,Y | -1,+1,-1 | → shoulder_pitch, roll, yaw |
| `rradius` | 右前臂 | X | +1 | → elbow |
| `rwrist` | 右手腕 | Y | -1 | → wrist_roll |
| `rhand` | 右手 | X,Z | -1,-1 | → wrist_pitch, wrist_yaw |

> ⚠️ CMU 骨骼中的 `head`、`lclavicle`、`ltoes`、`lfingers`、`lthumbs` 等部位在 G1 上没有对应关节，直接忽略。

### 符号（sign）为什么有 +1 和 -1？

CMU 和 G1 的关节旋转**正方向定义不同**：

- CMU 骨骼的旋转轴方向由 ASF 中的 `axis` 和 `direction` 字段定义
- G1 关节的旋转正方向由 URDF 中的 `<axis xyz="..."/>` 定义
- 两者不一定一致

例如：`lfemur` 的 X 轴正旋转在 CMU 中 = 向前抬腿，但 G1 `hip_pitch` 正值 = 向后伸腿，所以需要 `sign = -1`。

> 💡 这些符号都是通过**观察仿真中回放效果反复调试**得出的（代码注释中的 `[FIX]` 标记就是调试痕迹）。

### 坐标系转换（仅基座）

关节角是**无坐标系的弧度值**，不需要坐标变换。只有基座的位置和朝向需要从 CMU 坐标系转换到 Isaac 坐标系：

```
CMU 坐标系：X=右, Y=上, Z=后
Isaac 坐标系：X=前, Y=左, Z=上
```

$$\begin{aligned}
\text{Isaac}_x &= -\text{CMU}_z & \text{(前 = -后)} \\
\text{Isaac}_y &= -\text{CMU}_x & \text{(左 = -右)} \\
\text{Isaac}_z &= \text{CMU}_y & \text{(上 = 上)}
\end{aligned}$$

旋转也做同样的坐标变换：$R_{\text{Isaac}} = R_{\text{coord}} \cdot R_{\text{CMU}} \cdot R_{\text{coord}}^{-1}$

### 尺度缩放（仅基座位移）

CMU 被试者是真人体型（~1.7m），G1 是 ~1.2m 的机器人。关节角（弧度）与尺度无关，但**基座位移必须缩放**：

```python
# 自动校准：用 CMU 腿长 → G1 骨盆高度 (0.78m)
scale = G1_PELVIS_HEIGHT / (lfemur.length + ltibia.length)
# 所有 root 位移 × scale
isaac_pos = cmu_pos * scale
```
...
| 列 | 字段 | 含义 | 单位 | 说明 |
|----|------|------|------|------|
| 0 | base_pos_x | 骨盆位置 X（前方） | 米 | Isaac Z-up 坐标系 |
| 1 | base_pos_y | 骨盆位置 Y（左方） | 米 | |
| 2 | base_pos_z | 骨盆位置 Z（高度） | 米 | 站立时 ≈ 0.78m |
| 3 | quat_x | 骨盆朝向四元数 x | — | scipy xyzw 格式 |
| 4 | quat_y | 骨盆朝向四元数 y | — | |
| 5 | quat_z | 骨盆朝向四元数 z | — | |
| 6 | quat_w | 骨盆朝向四元数 w | — | 无旋转时 w≈1 |
| 7~12 | joint[0~5] | 左腿 6 关节角 | 弧度 | hip_pitch/roll/yaw, knee, ankle_pitch/roll |
| 13~18 | joint[6~11] | 右腿 6 关节角 | 弧度 | 同上 |
| 19~21 | joint[12~14] | 腰部 3 关节角 | 弧度 | waist_yaw/roll/pitch |
| 22~28 | joint[15~21] | 左臂 7 关节角 | 弧度 | shoulder×3, elbow, wrist×3 |
| 29~35 | joint[22~28] | 右臂 7 关节角 | 弧度 | 同上 |

> **四元数为什么不用欧拉角？** 欧拉角有万向节锁（Gimbal Lock）问题，四元数可以做球面线性插值（Slerp），帧间插值更平滑。CSV 中是 xyzw 格式（scipy 默认），`csv_to_npz.py` 加载后转为 wxyz（Isaac Lab 约定）。

> **关节角 = 0 表示默认站姿**。正/负值表示偏离默认姿态的方向和幅度，所有角度已经过 `wrap_to_pi` 包裹到 $[-\pi, \pi]$ 并 clamp 到 URDF 关节限位范围内。

## 2.3 阶段二：CSV → NPZ (生成算法直接可用的示教数据)

**脚本**：`scripts/mimic/csv_to_npz.py`

### CSV 有什么、缺什么？

CSV 只有 36 列（3 位移 + 4 朝向 + 29 关节角），但训练 reward 需要的远不止这些：

| Reward 函数 | 需要的参考数据 | CSV 有？ |
|------------|--------------|---------|
| `motion_joint_pos_error` | 29 个关节角 | ✅ |
| `motion_anchor_pos_error` | 骨盆世界坐标 | ✅ |
| `motion_anchor_ori_error` | 骨盆朝向 | ✅ |
| `motion_body_pos_error` | **30 个 body 的世界坐标** | ❌ |
| `motion_body_ori_error` | **30 个 body 的世界朝向** | ❌ |
| `motion_ee_pos_error` | **手脚末端世界坐标** | ❌ |
| `motion_lin_vel_error` | **骨盆线速度** | ❌ |
| `motion_ang_vel_error` | **骨盆角速度** | ❌ |

**结论**：CSV 缺少 body 世界坐标和所有速度数据，无法直接用于训练。

**为什么需要这一步并且必须重放？**
PPO 算法中的模仿学习（AMP / Mimic 奖励函数）在训练时，需要在每一个决策步（Step）极速拉取当前的**目标示教状态（含位置与速度）**，来计算自身姿态与标准武术动作的惩罚/奖励。
CSV 纯文本文件不但读取慢，且只包含了前一阶段映射出的“静态位置”。
因此 `.npz` 打包的意义在于：
1. **夹逼（Clamping）**：确保映射后的关节角度绝对没有超出真实 G1 机器人的机械限位（Joint Limits）。
2. **速度衍生（Velocity Derivation）**：单纯读取 CSV 逐帧的位移，根据微积分（差分法）计算出 `root_lin_vel` (基座世界系线速度)、`root_ang_vel` (基座角速度) 和 `joint_vel` (29个关节的角速度数据)。
3. **格式合规化**：输出 Numpy 多维张量数组，供 Isaac Lab 环境在 GPU 显存上零拷贝高速采样！

### 缺失数据如何生成？——Isaac Sim FK + 有限差分

NPZ 中的数据由**两种不同的方法**生成，不要混淆：

**① Isaac Sim 做的事（正向运动学 FK）——空间几何计算**：

给定关节角 → 算出每个 body 的**位置和朝向**。

```python
for each_frame in csv_data:
    # 1. 把 CSV 中的 root 位姿 + 29 个关节角写入仿真器
    robot.write_root_state_to_sim(root_state)
    robot.write_joint_state_to_sim(joint_pos_clamped, joint_vel_clamped)
    
    # 2. 只做渲染/FK，不跑物理！
    sim.render()  # ← 正向运动学：关节角 → body 世界坐标
    scene.update(dt)
    
    # 3. 读出仿真器 FK 算好的 30 个 body 的世界坐标和朝向
    log["body_pos_w"].append(robot.data.body_pos_w)    # (30, 3) 位置
    log["body_quat_w"].append(robot.data.body_quat_w)  # (30, 4) 朝向
```

**② 有限差分做的事（纯数学求导）——不需要仿真器**：

给定相邻帧的位置 → 算出速度。就是对 FK 结果在时间轴上求导：

$$v(t) \approx \frac{x(t+1) - x(t-1)}{2 \Delta t}$$

```python
# 关节角速度 = d(joint_pos) / dt
joint_vel = torch.gradient(joint_pos, spacing=dt)

# 骨盆线速度 = d(base_pos) / dt
base_lin_vel = torch.gradient(base_pos, spacing=dt)

# 骨盆角速度 = 从相邻帧四元数差求 axis-angle / dt
q_rel = q_next * q_prev⁻¹
omega = axis_angle(q_rel) / (2 * dt)
```
...
| NPZ 中的数据 | 怎么来的 | 谁算的 |
|-------------|---------|--------|
| `body_pos_w` (30×3) | 正向运动学 | **Isaac Sim** (sim.render) |
| `body_quat_w` (30×4) | 正向运动学 | **Isaac Sim** (sim.render) |
| `body_lin_vel_w` (30×3) | $(pos_{t+1}-pos_{t-1})/2\Delta t$ | **torch.gradient**（纯数学） |
| `body_ang_vel_w` (30×3) | $\text{axis\_angle}(q_{t+1} \cdot q_{t-1}^{-1})/2\Delta t$ | **torch.gradient**（纯数学） |
| `joint_vel` (29) | $(\theta_{t+1}-\theta_{t-1})/2\Delta t$ | **torch.gradient**（纯数学） |
| `joint_pos` (29) | 直接从 CSV 读取（经 clamp） | **CSV 原始数据** |

> 💡 Isaac Sim 只负责"关节角 → body 坐标"的几何计算（FK），所有速度都是拿到坐标之后用相邻帧做差除以时间间隔算出来的，和仿真器无关。

**为什么必须用仿真引擎做 FK（而不是自己手写）？**
...
2. **复杂模型**：G1 的 USD 模型有 30 个 body 各自的 mesh 偏移、质心偏移、固定关节合并等，只有用**同一个模型**做 FK，才能保证参考轨迹和训练数据完全一致。

### 帧率转换：CMU 120Hz → NPZ 50Hz

CMU 动捕数据采样率为 **120 Hz**，但训练的策略控制频率为 **50 Hz**：

$$\text{策略控制频率} = \frac{\text{物理仿真频率 (200Hz)}}{\text{decimation (4)}} = 50\text{Hz}$$

| 频率 | 值 | 含义 |
|------|-----|------|
| CMU 采集频率 | **120 Hz** | 光学动捕系统（Vicon）采样率 |
| 物理仿真频率 | **200 Hz** (dt=0.005s) | PhysX 物理引擎每秒解算步数 |
| 策略控制频率 | **50 Hz** (decimation=4) | 策略网络实际决策频率 |

NPZ 参考数据**必须与策略控制频率匹配（50Hz）**，否则参考轨迹和策略时间步不对齐，reward 计算会错乱。

`csv_to_npz.py` 内置了插值逻辑，在频率转换时：

- **位置和关节角**：线性插值（lerp）
- **四元数朝向**：球面线性插值（slerp），避免旋转跳变
- **速度**：从插值后的轨迹通过 `torch.gradient()` 有限差分计算

$$\underbrace{120\text{Hz}}_{\text{CMU 采集}} \xrightarrow[\text{lerp + slerp}]{\text{csv\_to\_npz.py}} \underbrace{50\text{Hz}}_{\text{NPZ 参考}} = \underbrace{\frac{200\text{Hz}}{4}}_{\text{策略控制频率}}$$
...
"29DOF" 指的是 **29 个可驱动关节的自由度**，全部为旋转（角度），不包含基座。基座（pelvis）在仿真中是浮动基座（floating base），其 6 个自由度（3 平移 + 3 旋转）由物理引擎根据地面反力和关节力矩自动求解，不作为策略的动作输出。

$$\text{Total DOF} = \underbrace{6}_{\text{base (浮动基座)}} + \underbrace{29}_{\text{joints (可控关节)}} = 35$$

但我们说"29DOF"是指**可控关节数 = 动作空间维度 = 29**。

**按身体部位分组**：

| 区域 | 每侧关节 | 数量 | 人体解剖学对应 |
|------|---------|------|--------------|
| **腿** | hip_pitch, hip_roll, hip_yaw, knee, ankle_pitch, ankle_roll | 6×2=**12** | 髋=球窝关节(3DOF), 膝=铰链(1DOF), 踝=万向节(2DOF) |
| **腰** | waist_yaw, waist_roll, waist_pitch | **3** | 腰椎简化为一个球窝关节 |
| **臂** | shoulder_pitch/roll/yaw, elbow, wrist_roll/pitch/yaw | 7×2=**14** | 肩=球窝(3DOF), 肘=铰链(1DOF), 腕=球窝(3DOF) |
| **合计** | | **29** | |

> 💡 **为什么各关节定义成这些 DOF？** 人体关节天然如此——髋和肩是球窝关节可三轴旋转，膝和肘主要做屈伸只需一轴，踝关节可做俯仰+侧摆（踮脚+内外翻）共两轴，腕关节可做三轴旋转。G1 的关节设计忠实复现了人体关节的运动学特性。

### SIM 关节顺序

在 Isaac Lab 仿真中，关节按照**交错的左-右-腰**顺序排列（SIM 关节顺序）：
